...
// Get all students with their graduation status
...
// Update a student's certificate status
...
export const getUniversityRankings = async () => {
  // Simulate API call delay
  await new Promise((resolve) => setTimeout(resolve, 700));
  return universityRankings;
};

// Generate new university rankings
export const generateUniversityRankings = async (
  ...
  // In a real app, this would generate new rankings
  // For now, we'll just return existing rankings
  const existingRanking = universityRankings.find(
    (r) => r.department === department && r.faculty === faculty
  );

  return (
    existingRanking || {
      id: String(universityRankings.length + 1),
      year,
      department,
      faculty,
      students: [],
...
// Get graduation decisions
export const getGraduationDecisions = async () => {
  // Simulate API call delay
  await new Promise((resolve) => setTimeout(resolve, 600));
  return graduationDecisions;
};

// Upload a new graduation decision
export const uploadGraduationDecision = async (decisionData: any) => {
  // Simulate API call delay
  ...
  return {
    success: true,
    id: String(graduationDecisions.length + 1),
    ...decisionData,
  };
};
