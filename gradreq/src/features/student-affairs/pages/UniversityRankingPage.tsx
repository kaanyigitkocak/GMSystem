...
const UniversityRankingPage = () => {
  ...
  const handleCreateRanking = async () => {
    ...
    setSuccess('');
    try {
      // Burada hem yüklenen dosyalar hem de seçilen sistem dosyaları kullanılabilir.
      // Şu an mock fonksiyon sadece yüklenen dosyaları birleştiriyor.
      // Gerçekleştirmek için, seçilen sistem dosyalarını da dahil etmek gerekir.
      // Şimdilik sadece yüklenen dosyalarla devam ediyoruz.
      const res = await createUniversityRanking();
      setResult(res);
      setSuccess('University ranking file generated successfully.');
    } catch (err: any) {
      setError(err.message || 'Failed to generate ranking.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Box>
      <Typography variant="h5" gutterBottom>Generate University Ranking</Typography>
      <Paper sx={{ p: 3, mb: 3 }}>
        <Typography sx={{ mb: 2 }}>
          You can either upload faculty ranking files (Excel/PDF) or select from the system's available faculty rankings.
        </Typography>
        ...
        <Typography variant="subtitle1" sx={{ mt: 2 }}>System Faculty Rankings:</Typography>
        <List>
          {systemFiles.length === 0 && <ListItem><ListItemText primary="No system faculty rankings available." /></ListItem>}
          {systemFiles.map(f => (
            <ListItem key={f.id}>
              <FormControlLabel
                control={
        ...
        {result && (
          <Box sx={{ mt: 3 }}>
            <Alert severity="success">
              <div>University ranking files are ready:</div>
              <div>
...
export default UniversityRankingPage; 